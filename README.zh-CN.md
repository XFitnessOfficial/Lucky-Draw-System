...
这个 repo 是前端。它预期有 **38 个 PostgreSQL 函式** 透过 PostgREST 曝露出来，而那些不包含在内。

这是刻意的。我们生产环境的函式写死了我们自己的认证流程、节流门槛和它们的失败模式 —— 公开那些等于公开我们家的门锁密码。我们公开的是**规格**。

**[`API_CONTRACT.md`](API_CONTRACT.md)** 列出全部 38 个函式、前端送出的确切参数，以及任何实作都必须遵守的六条规则。照着做，这个前端不用改一行就能跑。
